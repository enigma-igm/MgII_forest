...
savefig = 'paper_plots/cf_all.pdf'
...
xi_scale = 1
vmin, vmax = 0, 3500
ymin, ymax = -0.0013 * xi_scale, 0.0028 * xi_scale
...
ax1.text(2500, 0.85*ymax, r'$z < %0.2f$' % median_z, fontsize=xylabel_fontsize) #, linespacing=1.8)
...
ax2.text(2500, 0.85*ymax, r'$z \geq %0.2f$' % median_z, fontsize=xylabel_fontsize) #, linespacing=1.8)
...
ax3.text(2500, 0.85*ymax, r'All $z$', fontsize=xylabel_fontsize) #, linespacing=1.8)
...
ax4.fill_between(vel_mid_low, p5_mask_low, p95_mask_low, color='k', alpha=0.2, ec=None)

for iqso, xi in enumerate(xi_mask_all_low):
    ax4.plot(vel_mid_low, xi * xi_scale, linewidth=1.0, label=qso_namelist[iqso]) #, alpha=0.7) #c='tab:orange'

ax4.errorbar(vel_mid_low, xi_mean_mask_low * xi_scale, yerr=(xi_std_mask_low / np.sqrt(nqso)) * xi_scale, lw=2.0, \
             marker='o', c='black', ecolor='black', capthick=2.0, capsize=2, mec='none', zorder=20, label='all QSOs')

#ax4.text(2500, 0.85*ymax, r'$z < %0.2f$' % median_z, fontsize=xytick_size) #, linespacing=1.8)
ax4.legend(loc=1, fontsize=legend_fontsize)
ax4.set_xlabel(r'$\Delta v$ [km/s]', fontsize=xylabel_fontsize)
ax4.set_ylabel(r'$\xi(\Delta v)$', fontsize=xylabel_fontsize)
vel_doublet = 768.469
ax4.axvline(vel_doublet, color='green', linestyle='--', linewidth=2.0)
ax4.set_ylim([ymin, ymax])
ax4.xaxis.set_minor_locator(AutoMinorLocator())
ax4.yaxis.set_minor_locator(AutoMinorLocator())
ax4.tick_params(top=True, which='both', labelsize=xytick_size)

ax5.fill_between(vel_mid_high, p5_mask_high, p95_mask_high, color='k', alpha=0.2, ec=None)

for xi in xi_mask_all_high:
    ax5.plot(vel_mid, xi * xi_scale, linewidth=1.0) #, alpha=0.7) #c='tab:orange'

ax5.errorbar(vel_mid, xi_mean_mask_high * xi_scale, yerr=(xi_std_mask_high / np.sqrt(nqso)) * xi_scale, lw=2.0, \
             marker='o', c='black', ecolor='black', capthick=2.0, capsize=2, mec='none', zorder=20)

#ax5.text(2500, 0.85*ymax, r'$z \geq %0.2f$' % median_z, fontsize=xytick_size) #, linespacing=1.8)
ax5.set_xlabel(r'$\Delta v$ [km/s]', fontsize=xylabel_fontsize)
ax5.axvline(vel_doublet, color='green', linestyle='--', linewidth=2.0)
ax5.set_ylim([ymin, ymax])
ax5.xaxis.set_minor_locator(AutoMinorLocator())
ax5.yaxis.set_minor_locator(AutoMinorLocator())
ax5.tick_params(top=True, which='both', labelsize=xytick_size)

ax6.fill_between(vel_mid, p5_mask_all, p95_mask_all, color='k', alpha=0.2, ec=None)

for xi in xi_mask_all:
    ax6.plot(vel_mid, xi * xi_scale, linewidth=1.0) #, alpha=0.7) #c='tab:orange'

ax6.errorbar(vel_mid, xi_mean_mask_high * xi_scale, yerr=(xi_std_mask / np.sqrt(nqso)) * xi_scale, lw=2.0, \
             marker='o', c='black', ecolor='black', capthick=2.0, capsize=2,  mec='none', zorder=20)

#ax6.text(2500, 0.85*ymax, r'All $z$', fontsize=xytick_size) #, linespacing=1.8)
ax6.set_xlabel(r'$\Delta v$ [km/s]', fontsize=xylabel_fontsize)
ax6.axvline(vel_doublet, color='green', linestyle='--', linewidth=2.0)
