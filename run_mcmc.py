...
modelfile = mgii_dir + 'igm_cluster/10qso/rebin/corr_func_models_%s_ivarweights.fits' % redshift_bin

subarr_flag = True # flag to extract sub arrays and do inference using velocity lag bins that are not masked
nsteps = 300000 #100000
burnin = 30000 #10000
nwalkers = 40

#########################
if subarr_flag:
    #figpath = mgii_dir + 'mcmc/10qso-new/paper/xi_mask_extract_subarr2/'
    figpath = mgii_dir + 'mcmc/10qso-new/paper/xi_mask_extract_subarr2/test-highz/'
else:
    figpath = mgii_dir + 'mcmc/10qso-new/paper/xi_fullarr/'

#########################
if redshift_bin == 'all':
    cf = fits.open('save_cf/paper_new/xi_10qso_everyn60_corr_allz.fits') #('save_cf/paper/xi_mean_mask_10qso_everyn60_corr.fits')
    if subarr_flag:
        covar_fine_file = mgii_dir + 'igm_cluster/10qso/rebin/covar_fine_all_ivarweights_subarr.npy'
        lag_mask, _ = mutils.cf_lags_to_mask()
    else:
        covar_fine_file = mgii_dir + 'igm_cluster/10qso/rebin/covar_fine_all_ivarweights.npy'
        lag_mask = None

elif redshift_bin == 'high':
    cf = fits.open('save_cf/paper_new/xi_10qso_everyn60_corr_highz.fits') #('save_cf/paper/xi_mean_mask_10qso_everyn60_highz.fits')
    if subarr_flag:
        covar_fine_file = mgii_dir + 'igm_cluster/10qso/rebin/covar_fine_high_ivarweights_subarr.npy'
        lag_mask, _ = mutils.cf_lags_to_mask_highz()
    else:
        covar_fine_file = mgii_dir + 'igm_cluster/10qso/rebin/covar_fine_high_ivarweights.npy'
        lag_mask = None

elif redshift_bin == 'low':
    cf = fits.open('save_cf/paper_new/xi_10qso_everyn60_corr_lowz.fits') #('save_cf/paper/xi_mean_mask_10qso_everyn60_lowz.fits')
    if subarr_flag:
        covar_fine_file = mgii_dir + 'igm_cluster/10qso/rebin/covar_fine_low_ivarweights_subarr.npy'
...
sampler, param_samples, flat_samples, extra_out = mcmc.run_mcmc(fine_out, coarse_out, data_out, redshift_bin, figpath, \
                                                     nsteps=nsteps, burnin=burnin, nwalkers=nwalkers, \
...
if subarr_flag:
    mcmc_out_full_arr = mgii_dir + 'mcmc/10qso-new/paper/xi_fullarr/%sz_mcmc_chain.fits' % redshift_bin
